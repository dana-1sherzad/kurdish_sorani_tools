...
import '/src/constants/words_to_number/constants.dart';
...
int _compute(List<String> tokens) {
  ...
  for (var element in tokens) {
    final token = convertKuToEn(element);
...
String? wordsToNumberString(
  ...
  final addedCommasIfNeeded =
      addComma ? computeNumbers.ZyadkrdnyFariza : computeNumbers.toString();

  switch (digits) {
    case DigitLocale.ku:
      return convertEnToKu(addedCommasIfNeeded);
    case DigitLocale.ckb:
      return convertEnToKu(addedCommasIfNeeded);
    case DigitLocale.en:
    default:
